...
samples = [f'R00{str(i).zfill(2)}' for i in range(1,23)]
...
rule snippy:
    input:
        ref = "scratch/fungus_2/annotation/fungus_2.emapper.decorated.gbk",
    ...
    shell:
        """
        ml snippy
        snippy --outdir {params.outdir} --ref {input.ref} --R1 {input.r1} --R2 {input.r2} --cpus 32 --prefix {wildcards.sample}_ref --force --report
        ml purge
        """

rule nucmer:
    input:
        ref = "scratch/fungus_2/assembly/assembly.fasta",
...
rule delly:
    input:
        ...
        r1 = "scratch/{sample}/qc/{sample}_R1.fq.gz",
        r2 = "scratch/{sample}/qc/{sample}_R2.fq.gz",
        map = "scratch/fungus_2/assembly/map.fa.gz"
    output:
